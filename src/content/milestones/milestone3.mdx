...
- On the right is a run button from Eclipse IDE, a popular tool for programming in Java. On the left is GrafState’s current run button.
  ![Run Buttons](/run-buttons.jpg)
- Below on the far left is Grafstate’s back button, and we found the other buttons frequently on similar websites.
...
- Adjusting the presentation of the buttons may not be enough to convey its functionality, because Grafstate offers unique features such as inserting prewritten code onto the terminal. Adding small text labels on some of the buttons may help users understand its functionality. 
...
- This idea combines a few that we came up with during the brainstorming session. Compared to merely adding a section to the documentation explaining the site or making a video walkthrough, this idea will engage the user more by prompting them to go through the steps of running the simulation themselves.
...
- Introducing color to the user interface is practically supported in design research and helps facilitate our goal of making a visually appealing and helpful interface. An experiment evaluating color on human affective cognition in dynamic web pages concluded that “the results obtained in the field of color design considerably differ from the results of this study, which indicate that dark royal blue (#4169e1) and dark cyan ( #006400), which are cold colors, are the most suitable colors for web page design” (Lungwen, et al). Due to these findings we used royal blue for the zoom, insert, logout, and exit buttons and dark cyan for the run icon is an aesthetic decision for Grafstate. A research article published by Lisa Wilms and Daniel Oberfeld confirms “that color has systematic effects on the emotional state of a person viewing the color.” Consistently using desaturated colors, especially the gray tones present in the old interface, make the buttons visually uninteresting and harder to look for in a monochromatic color scheme. The choice to switch to the blue colors help information and important icons jump out at the user, making it easier for a user to intuitively know what to do. The cold colors, such as the royal blue, consistently had the most positive results from the study and were deemed as most suitable for the tourism website used in the study. Although the study comes with limitations, “...the experimental results provide a theoretical and practical reference for the selection of appropriate colors and design styles for tourism websites and web pages of other product types.” 

- Additionally, the decision to round the icon’s borders and keep the background in grayscale increases visibility and reduces search time. Contrast can play a role in affecting emotion, and a one study concludes that “the enhancement of contrast induces the increase of valence, while the reduction of contrast induces the decrease,” which suggests higher contrast design in websites will have positive effects on user experience (Yang, et al). Therefore, we chose grayscale for the interface background in order to keep contrast between backgrounds, buttons, and text high. This decision is supported by another study that says “interface background color plays an important role in visual search, and high color differentiation can improve icon recognition performance” (Deng et al pg 3). This study looks into how the user visual experience and search time is affected by an icon shape and background color choice, and it concludes that RS (rounded shape) icons paired with LS (low saturation) backgrounds “…may produce the best user visual experience and the most efficient search effectiveness” based on its findings (Deng, et al pg 11). 

**[Figma Link](https://www.figma.com/file/fyyYWh587S1UZoT5rDQVsc/High-Fidelity---User-Story-1?t=pG16RhdV0VKYUsfq-0)**
...
**[Figma Link](https://www.figma.com/file/izF1pLqMP5d736LssGFaxE/High-Fidelity---User-Story-2?t=k07fFvYGopA2L3aN-0)**
...
- We decided to go with this design as we wanted users to be able to quickly become acquainted with the new layout whether they are first time or returning users. We felt the quickest way to introduce how the UX for creating computational structures works is to introduce a simple and optional step that demonstrates how to do so. We found that such a solution is a the most ideal way to expand a user’s repertoire of skills (Lafreniere et al. 2013).
- Another reason we went with this design was we wanted a coherent user experience that fit with the rest of the design language of the new layout. Following the golden rule of striving for consistency allows for similar signifiers to be present throughout the design leading to a better overall user experience (Shneiderman et al. 2017).
...
**[Figma Link](https://www.figma.com/file/PvcgpLXplS8xc9yk6JAyzY/High-Fidelity---User-Story-3?t=k07fFvYGopA2L3aN-0)**
